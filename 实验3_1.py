...
class BankerAlgorithmGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Button(control_frame, text="完成进程并回收资源",
                   command=self.complete_process).grid(row=0, column=4, padx=5)
        ttk.Button(control_frame, text="重置系统",
                   command=self.reset_system).grid(row=0, column=5, padx=5)
    ...
    def process_request(self):
        ...
        if dialog.result:
            ...
            if is_safe:
                ...
                # 检查进程是否完成（Need全为0）
                if all(n == 0 for n in self.Need[pid]):
                    self.complete_process_by_id(pid)
            else:
                # 恢复原始状态
    ...
    def complete_process(self):
        ...
        dialog = CompleteProcessDialog(self.root, self.n)
        if dialog.result:
            pid = dialog.result
            ...
            if all(n == 0 for n in self.Need[pid]):
                ...
                messagebox.showwarning("完成进程", f"进程 P{pid} 尚未满足其所有需求，无法完成")
                self.log(f"尝试完成进程 P{pid} 失败：进程尚未满足所有需求")

    def complete_process_by_id(self, pid):
        """完成指定进程并回收其资源"""
        # 回收进程占用的所有资源
        for j in range(self.m):
        ...
        self.log(f"进程 P{pid} 已完成，资源已回收，Available = {self.Available}")
        messagebox.showinfo("完成进程",
                            f"进程 P{pid} 已完成\n已回收资源: {self.Allocation[pid]}\n当前可用资源: {self.Available}")
    ...
    def update_status_display(self):
        ...
        self.status_text.insert(tk.END, "=" * 100 + "\n")

        # 表头
        header = f"{'进程':<4} {'Max':<15} {'Allocation':<15} {'Need':<15} {'Available':<15}\n"
        ...
        for i in range(self.n):
            ...
            # 对齐文本
            max_str = max_str.ljust(15)
            alloc_str = alloc_str.ljust(15)
            need_str = need_str.ljust(15)

            if i == 0:
                avail_str = " ".join(f"{x:2d}" for x in self.Available)
                avail_str = avail_str.ljust(15)
                self.status_text.insert(tk.END, f"P{i:<3} {max_str} {alloc_str} {need_str} {avail_str}\n")
            else:
                self.status_text.insert(tk.END, f"P{i:<3} {max_str} {alloc_str} {need_str}\n")
...
class ManualInputDialog:
    ...
    def setup_ui(self):
        ...
        ttk.Label(input_frame, text="\n已分配矩阵 (Allocation):", font=("Arial", 10, "bold")).grid(
            row=self.n + 3, column=0, columnspan=self.m + 2, sticky=tk.W, pady=(20, 10))

        # 表头
        ttk.Label(input_frame, text="进程\\资源").grid(row=self.n + 4, column=0, padx=5, pady=2, sticky=tk.W)
        for j in range(self.m):
            ttk.Label(input_frame, text=f"R{j}").grid(row=self.n + 4, column=j + 1, padx=5, pady=2)

        self.allocation_entries = []
        for i in range(self.n):
            ttk.Label(input_frame, text=f"P{i}").grid(row=self.n + 5 + i, column=0, padx=5, pady=2, sticky=tk.W)
            row_entries = []
            for j in range(self.m):
                entry = ttk.Entry(input_frame, width=8)
                entry.grid(row=self.n + 5 + i, column=j + 1, padx=2, pady=2)
                row_entries.append(entry)
            self.allocation_entries.append(row_entries)

        # 可用资源向量
        ttk.Label(input_frame, text="\n可用资源向量 (Available):", font=("Arial", 10, "bold")).grid(
            row=2 * self.n + 6, column=0, columnspan=self.m + 2, sticky=tk.W, pady=(20, 10))
        ...
        button_frame.grid(row=2 * self.n + 9, column=0, columnspan=self.m + 2, pady=20)
...
class CompleteProcessDialog:
    ...
    def setup_ui(self):
        ...
        # 选择进程
        ttk.Label(frame, text="选择要完成的进程:").grid(row=0, column=0, sticky=tk.W, pady=5)
        self.process_var = tk.StringVar()
        process_combo = ttk.Combobox(frame, textvariable=self.process_var,
                                     values=[f"P{i}" for i in range(self.n)], state="readonly")
        process_combo.grid(row=0, column=1, sticky=tk.W, pady=5)
